...
CAP_NAME=${1:-dpi-baseline.pcap}
TEXT_NAME=${2:-${CAP_NAME}.txt}
SUMMARY_NAME=${3:-${CAP_NAME}.sni.txt}
PCAP_PATH="$CAP_DIR/$CAP_NAME"
TEXT_PATH="$CAP_DIR/$TEXT_NAME"
SUMMARY_PATH="$CAP_DIR/$SUMMARY_NAME"
if [ ! -f "$PCAP_PATH" ]; then
...
echo "[export] TLS-decoded text: $TEXT_PATH"
echo "[export] SNI summary: $SUMMARY_PATH"
